...
public class UsbMjpegServer extends NanoHTTPD {
    ...
    private final BlockingQueue<byte[]> frameQueue = new ArrayBlockingQueue<>(4);

    public UsbMjpegServer() {
        super("127.0.0.1", 8088);
    }

    /** Called by UsbCameraPlugin for every new MJPEG frame from the capture card. */
    public void pushFrame(byte[] jpegData) {
        if (jpegData == null || jpegData.length == 0) return;
        while (frameQueue.remainingCapacity() == 0) frameQueue.poll();
    ...
    public Response serve(IHTTPSession session) {
        if ("/stream".equals(session.getUri())) {
            Response r = newChunkedResponse(
                ...
                new MjpegInputStream(frameQueue, BOUNDARY)
            );
    ...
    private static class MjpegInputStream extends InputStream {
        ...
        MjpegInputStream(BlockingQueue<byte[]> q, String b) { this.queue = q; this.boundary = b; }
        ...
        private void loadNextChunk() throws IOException {
            try {
                byte[] frame = queue.poll(1000, TimeUnit.MILLISECONDS);
                if (frame == null) { chunk = null; return; }
                String header = "--" + boundary + "\\r\\nContent-Type: image/jpeg\\r\\n"
                    + "Content-Length: " + frame.length + "\\r\\n\\r\\n";
                byte[] hb = header.getBytes("UTF-8");
                byte[] tail = "\\r\\n".getBytes("UTF-8");
...
public class UsbCameraPlugin extends Plugin {
    ...
    // â”€â”€ USB Streaming Setup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    /**
     * UVC Probe/Commit handshake â€” tells the card we want MJPEG 1920x1080 @ 60fps.
    ...
    private void negotiateFormat(UsbDeviceConnection conn, int ifaceId) {
        ...
        if (interval > 0) {
            int fps = (int) Math.round(10_000_000.0 / interval);
            Log.d(TAG, "Negotiated MJPEG " + TARGET_W + "x" + TARGET_H + " @ " + fps + "fps");
        }
    }

    // â”€â”€ Bulk Frame Reader â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
